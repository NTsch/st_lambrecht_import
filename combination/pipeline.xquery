...
(: insert data from the spreadsheet
...
let $to-delete := $charter//cei:chDesc//cei:figure
return (
  delete nodes $to-delete,
  for $url in tokenize($match/cell[@n='2']/text(), '\$')
  let $figure := <cei:figure><cei:graphic url="{$url}"/></cei:figure>
  return if ($charter//cei:witnessOrig) then 
  (insert node $figure into $charter//cei:witnessOrig)
  else (insert node $figure into $charter//cei:witness[1])
)

(: replace dates :)
(: where $charter//cei:issued/cei:date
